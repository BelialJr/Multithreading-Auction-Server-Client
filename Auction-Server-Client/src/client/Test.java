...
public class Test extends VBox {
    ...
    private DoubleProperty angle = new SimpleDoubleProperty(Math.toDegrees(2 * Math.PI / 60));
    ...
    public Test(final JFXTimePicker jfxTimePicker) {
    ...
    protected BorderPane createContentPane(LocalTime time, boolean _24HourView) {
        ...
        hoursContent = createMinutesContent(time);
    ...
    void updateValue() {
        if (is24HourView) {
            ...
            timePicker.setValue(LocalTime.parse(selectedHourLabel.getText() + ":" + selectedMinLabel.getText(), DateTimeFormatter.ofPattern("h:mm").withLocale(Locale.ENGLISH)));
        }
    }
    ...
    private void goToTime(LocalTime time) {
        if (time != null) {
            ...
            minsPointerRotate.setAngle(180 + (time.getMinute() + 45) % 60 * Math.toDegrees(2 * Math.PI / 60));
            hoursPointerRotate.setAngle(180 + (time.getMinute() + 45) % 60 * Math.toDegrees(2 * Math.PI / 60));
